...
## Polígonos NSE
  ...
  kml_layers <- st_layers('/Users/yalta/Library/CloudStorage/GoogleDrive-yaltalielt@gmail.com/Mi unidad/4S Real Estate/2025/[02] ALGORITMO/[02] DATOS/[02] KML_KMZ/NSE/NSE-Monterrey.kml')
  print(kml_layers)
  
  # 2. Leer cada capa y guardarlas en una lista
  kml_data_list <- lapply(kml_layers$name, function(x) {
    st_read('/Users/yalta/Library/CloudStorage/GoogleDrive-yaltalielt@gmail.com/Mi unidad/4S Real Estate/2025/[02] ALGORITMO/[02] DATOS/[02] KML_KMZ/NSE/NSE-Monterrey.kml', layer = x)
  })
...
# --- Predicciones ----
  ...
  nueva_obs <- data.frame(
    distancia_parque   = 250,
    Absorcion          = 12,
    m2_inv             = 850,
    Segmento           = "Residencial",
    unidades_vendidas  = 25,
    NSE                = "B",
    Precio_Prom_Vivienda = 5800000
  )
...
# --- Termómetro Calificador ----
  ...
  ## 9. Graficar
  ggplot() +
    ### 9.1. Rectángulos de colores para el termómetro
    geom_rect(
    ...
    ### 9.2. Eje “horizontal” girado
    coord_flip() +
    ### 9.3. Agregar la etiqueta al interior de cada rectángulo
    geom_text(
      data = termometro_data,
      aes(x = 1, y = (ymin + ymax) / 2, label = rango)
    ) +
    ### 9.4. Agregar las líneas para cada proyecto
    geom_hline(
    ...
    ### 9.5. Agregar el texto (nombre del proyecto) cerca de la línea
    geom_text(
      data = percentiles_clientes_gbm_termometro,
    ...
    ### 9.6. Escala de color para proyectos
